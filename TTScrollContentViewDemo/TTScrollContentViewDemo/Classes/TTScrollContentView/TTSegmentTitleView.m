...
#import "TTSegmentTitleView.h"

@interface TTSegmentTitleView ()<UIScrollViewDelegate>
...
@property (nonatomic, assign) TTIndicatorType indicatorType;

@property (nonatomic, strong) NSArray *titlesArr;

@end

@implementation TTSegmentTitleView
...
- (void)layoutSubviews {
    ...
    if (_titleFont != _titleSelectFont) {
        for (int idx = 0; idx < self.titlesArr.count; idx++) {
            ...
            CGFloat itemBtnWidth = [TTSegmentTitleView getWidthWithString:self.titlesArr[idx] font:titleFont] + self.itemMargin;
            totalBtnWidth += itemBtnWidth;
        }
    } else {
        for (NSString *title in self.titlesArr) {
            CGFloat itemBtnWidth = [TTSegmentTitleView getWidthWithString:title font:titleFont] + self.itemMargin;
            totalBtnWidth += itemBtnWidth;
        }
    }
...
- (void)moveIndicatorView:(BOOL)animated {
    ...
    CGFloat indicatorWidth = [TTSegmentTitleView getWidthWithString:self.titlesArr[self.selectIndex] font:titleFont];
    [UIView animateWithDuration:(animated?0.05:0) animations:^{
        switch (self->_indicatorType) {
            case TTIndicatorTypeDefault:
                ...
                break;
            case TTIndicatorTypeEqualTitle:
...
- (void)btnClick:(UIButton *)btn {
    ...
    if (self.delegate&&[self.delegate respondsToSelector:@selector(TTSegmentTitleView:startIndex:endIndex:)]) {
        [self.delegate TTSegmentTitleView:self startIndex:self.selectIndex endIndex:index];
    }
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    if (self.delegate&&[self.delegate respondsToSelector:@selector(TTSegmentTitleViewWillBeginDragging:)]) {
        [self.delegate TTSegmentTitleViewWillBeginDragging:self];
    }
}

- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset {
    if (self.delegate&&[self.delegate respondsToSelector:@selector(TTSegmentTitleViewWillEndDragging:)]) {
        [self.delegate TTSegmentTitleViewWillEndDragging:self];
    }
}
