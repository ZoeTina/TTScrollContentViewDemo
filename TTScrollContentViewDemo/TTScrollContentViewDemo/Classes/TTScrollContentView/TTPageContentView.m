...
@interface TTPageContentView ()<UICollectionViewDelegate,UICollectionViewDataSource>
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(TTContentViewWillBeginDragging:)]) {
        [self.delegate TTContentViewWillBeginDragging:self];
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(TTContentViewDidScroll:startIndex:endIndex:progress:)]) {
        [self.delegate TTContentViewDidScroll:self startIndex:startIndex endIndex:endIndex progress:progress];
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(TTContenViewDidEndDecelerating:startIndex:endIndex:)]) {
        [self.delegate TTContenViewDidEndDecelerating:self startIndex:startIndex endIndex:endIndex];
    }
}

- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    if (!decelerate) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(TTContenViewDidEndDragging:)]) {
            [self.delegate TTContenViewDidEndDragging:self];
        }
    }
}
